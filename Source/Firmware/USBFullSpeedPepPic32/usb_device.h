...
typedef enum
{
    ...
    the D+ or D- line.  */
    DETACHED_STATE = 0x00,
    /* Attached is the state in which the device is attached on the bus but the
...
/* USB device stack events description */
typedef enum
{
...
/********************************************************************
    ...
    Parameters:
        UINT8 ep - The endpoint number that should be configured to send STALL.
        UINT8 dir - The direction of the endpoint to STALL, either 
...
/**************************************************************************
    ...
    Parameters:
        UINT8 endpoint - the endpoint number you wish to cancel the transfers for
...
/*************************************************************************
  ...
  Description:
    ...
    if(!USBHandleBusy(USBGenericInHandle))
    {
        ...
        USBGenericInHandle = USBGenWrite(USBGEN_EP_NUM,(UINT8*)&INPacket[0],sizeof(INPacket));
    }
    </code>
...
/********************************************************************
    Function:
        UINT16 USBHandleGetLength(USB_HANDLE handle)
...
UINT16 USBHandleGetAddr(USB_HANDLE);

#define USBHandleGetAddr(handle) ConvertToVirtualAddress((((volatile BDT_ENTRY*)handle)->ADR))

/********************************************************************
    Function:
        USB_HANDLE USBGetNextHandle(UINT8 ep_num, UINT8 ep_dir)
    Summary:
    ...
    Parameters:
        UINT8 ep_num - The endpoint number to get the handle for (valid 
                       values are 1-15,  0 is not a valid input value for this API)
        UINT8 ep_dir - The endpoint direction associated with the endpoint number 
    ...
    Remarks:
        ...
        UINT8 UserDataBuffer1[64];
        UINT8 UserDataBuffer2[64];
        UINT8* pDataBuffer = &UserDataBuffer1[0];
...
/********************************************************************
    Function:
        void USBEP0Transmit(UINT8 options)
...
/*************************************************************************
  Function:
        void USBEP0SendRAMPtr(UINT8* src, WORD size, UINT8 Options)
...
//void USBEP0SendRAMPtr(UINT8* src, UINT16 size, UINT8 Options);

#define USBEP0SendRAMPtr(src, size, options) {\
inPipes[0].pbSrc.bRam = src; \
inPipes[0].wCount = size; \
inPipes[0].info.Val = options | USB_EP0_BUSY | USB_EP0_RAM; \
}

/**************************************************************************
  Function:
        void USBEP0SendROMPtr(UINT8* src, WORD size, UINT8 Options)
...
//void USBEP0SendROMPtr(UINT8* src, UINT16 size, UINT8 Options);

#define USBEP0SendROMPtr(src, size, options) { \
inPipes[0].Source.pbRom = src; \
inPipes[0].wCount = size; \
...
/***************************************************************************
  Function:
    void USBEP0Receive(UINT8* dest, WORD size, void (*function))
  Summary:
...
//void USBEP0Receive(UINT8* dest, UINT16 size, void (*function));
...
/********************************************************************
    Function:
        USB_HANDLE USBTxOnePacket(UINT8 ep, UINT8* data, WORD len)
...
//USB_HANDLE USBTxOnePacket(UINT8 ep, UINT8* data, UINT16 len);

#define USBTxOnePacket(ep, data, len) USBTransferOnePacket(ep, IN_TO_HOST, data, len)

/********************************************************************
    Function:
        USB_HANDLE USBRxOnePacket(UINT8 ep, UINT8* data, UINT16 len)
...
/*******************************************************************************
  Function:
    BOOL USB_APPLICATION_EVENT_HANDLER(UINT8 address, USB_EVENT event, void *pdata, WORD size);
  ...
  Input:
    UINT8 address -  the address of the device when the event happened
    UINT8 event   -  The event input specifies which event happened.  The
...
BOOL USB_APPLICATION_EVENT_HANDLER(UINT8 address, USB_EVENT event, void *pdata, UINT16 size);
...
typedef struct __attribute__ ((packed)) tagTInPipe
{
    ...
    UINT16 __attribute__((aligned)) wCount;
} TInPipe;

extern USB_VOLATILE TInPipe inPipes[];

typedef struct __attribute__ ((packed)) tagTOutPipe
{
    union __attribute__ ((packed))
    {
        //Various options of pointers that are available to
