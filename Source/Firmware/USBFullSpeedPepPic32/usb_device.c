...
per 100us, or preferably faster).  Therefore, in all applications, it is 
...
static USB_VOLATILE UINT8 l_USBActiveConfiguration;
...
USB_VOLATILE TInPipe inPipes[1];
USB_VOLATILE TOutPipe outPipes[1];
...
USB_VOLATILE TEndPointStatus ep_data_in[USB_MAX_EP_NUMBER + 1];
USB_VOLATILE TEndPointStatus ep_data_out[USB_MAX_EP_NUMBER + 1];
 ...
 *******************************************************************/
volatile TUsbSetupPacket SetupPkt CTRL_TRF_SETUP_ADDR_TAG;
volatile UINT8 CtrlTrfData[USB_EP0_BUFF_SIZE] CTRL_TRF_DATA_ADDR_TAG;
...
//Device descriptor
extern ROM TUsbDeviceDescriptor g_DeviceDescriptor;

//Array of configuration descriptors
extern ROM UINT8* ROM USB_CD_Ptr[];

extern ROM UINT8* ROM USB_SD_Ptr[];
...
/****************************************************************************
  ...
  ***************************************************************************/
#define USBAdvancePingPongBuffer(buffer) { *((UINT8*)buffer) ^= USB_NEXT_PING_PONG; }
#define USBHALPingPongSetToOdd(buffer)   { *((UINT8*)buffer) |= USB_NEXT_PING_PONG; }
#define USBHALPingPongSetToEven(buffer)  { *((UINT8*)buffer) &= ~USB_NEXT_PING_PONG; }
  ...
  Description:
    ...
    if(USBGetDeviceState() == CONFIGURED_STATE)
    {
        ...
        if(!USBHandleBusy(USBInHandle))
        {
            ...
            USBInHandle = USBTransferOnePacket(EP_NUM,IN_TO_HOST,(UINT8*)&INPacket[0],sizeof(INPacket));
        }
    }
    </code>
...
/********************************************************************
 * Function:        void USBConfigureEndpoint(UINT8 EPNum, UINT8 direction)
 *
 * PreCondition:    None
 *
 * Input:           UINT8 EPNum - the endpoint to be configured
 *                  UINT8 direction - the direction to be configured
