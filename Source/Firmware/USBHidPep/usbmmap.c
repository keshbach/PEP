...
#pragma udata
UINT8 g_nUsbDeviceState;             // Device States: DETACHED, ATTACHED, ...
USB_DEVICE_STATUS g_UsbDeviceStatus; // Global USB flags
UINT8 g_nUsbActiveCfg;               // Value of current configuration
UINT8 g_nUsbAlternateInterface[MAX_NUM_INT]; // Array to keep track of the current alternate
...
volatile far BDT ep0Bo;         // Endpoint #0 BD Out (Memory Address: 0x0200)
volatile far BDT ep0Bi;         // Endpoint #0 BD In  (Memory Address: 0x0204)
volatile far BDT ep1Bo;         // Endpoint #1 BD Out (Memory Address: 0x0208)
volatile far BDT ep1Bi;         // Endpoint #1 BD In  (Memory Address: 0x020C)
 ...
 *****************************************************************************/
volatile far CTRL_TRF_SETUP SetupPkt;   // Memory Address: 0x0210
volatile far CTRL_TRF_DATA CtrlTrfData; // Memory Address: 0x0218

volatile far UINT8 g_nHidReportOut[HID_INT_OUT_EP_SIZE]; // Memory Address: 0x0220
volatile far UINT8 g_nHidReportIn[HID_INT_IN_EP_SIZE];   // Memory Address: 0x0260
